...
@app.route('/dashboard')
def dashboard():
    # 移除jwt_required裝飾器，改為在函數內檢查token
    token = request.args.get('token')
    if not token:
        # 如果沒有token，檢查session中是否有user_id
...
def login():
    try:
        ...
        if request.is_json:
            data = request.get_json()
            app.logger.info("接收到JSON登入請求")
        else:
            data = request.form
            app.logger.info("接收到表單登入請求")
            
        username = data.get('username')
        password = data.get('password')

        app.logger.info(f"嘗試登入用戶: {username}")
        ...
        try:
            with conn.cursor() as cursor:
                ...
                try:
                    ...
                    app.logger.debug(f"存儲的密碼雜湊: {stored_pw[:10]}... (長度: {len(stored_pw)})")
                    ...
                    try:
                        ...
                        app.logger.error(f"BCrypt錯誤(值錯誤): {str(ve)}")
                        return jsonify({'status': 'error', 'message': '身份驗證錯誤'}), 500
                    except Exception as e:
                        app.logger.error(f"密碼驗證錯誤: {str(e)}")
                        # 嘗試使用替代方法驗證
                        app.logger.info("嘗試使用替代方法驗證密碼")
                        if stored_pw == password:  # 純文本比較 (臨時措施，僅用於診斷)
                            app.logger.warning("使用純文本密碼登入成功，建議更新密碼雜湊")
                            session['user_id'] = user['id']
                            session['username'] = username
                            token = create_access_token(identity=username)
                    ...
                    app.logger.error(f"身份驗證過程中發生錯誤: {str(auth_error)}")
                    return jsonify({'status': 'error', 'message': '伺服器錯誤，請稍後再試'}), 500
        finally:
            conn.close()
    except Exception as e:
